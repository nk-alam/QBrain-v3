...
const JoinTeamPage = () => {
  ...
  const handleNext = () => {
    ...
    if (currentStep === 1) {
      ...
      if (!applicationData.personalInfo || !applicationData.isValid) {
        toast.error('Please fill in all required fields correctly');
        return;
      }
    } else if (currentStep === 2) {
      // Check if quiz is completed and passed
      if (!applicationData.quizResults || !applicationData.quizResults.passed) {
        toast.error('Please complete the quiz with a minimum score of 70%');
        return;
      }
                  ...
                  <div className="flex justify-between items-center pt-6 border-t border-slate-700/50">
                    ...
                    <button
                      ...
                      disabled={currentStep === steps.length || 
                        (currentStep === 1 && (!applicationData.personalInfo || !applicationData.isValid)) ||
                        (currentStep === 2 && (!applicationData.quizResults || !applicationData.quizResults.passed)) ||
                        (currentStep === 3 && !applicationData.resumeFile)
                      }
                      className={`flex items-center px-6 py-3 rounded-full transition-all duration-300 ${
