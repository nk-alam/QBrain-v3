  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingAchievement) {
        const result = await updateAchievement(editingAchievement.id, achievementData, imageFiles);
        if (result.success) {
        ...
        const result = await addAchievement(achievementData, imageFiles);
        if (result.success) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="text-xs text-gray-400 mt-1">
                Upload multiple images (certificates, team photos, project demos, etc.)
              </div>
            </div>
